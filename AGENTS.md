# Agent Guide — ep_pad_activity_nofication_in_title
...
`ep_pad_activity_nofication_in_title` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_pad_activity_nofication_in_title`:
...
* Client: `postAceInit`, `aceEditEvent`
